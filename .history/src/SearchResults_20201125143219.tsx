      ...
      <ul className='search__results card'>
          {results.map((result,i) => 
            ...
            <div className='card__block-media'>
              <img className='card__img' alt={result.original_name} src={
                result.poster_path !== null && result.poster_path !== undefined ? `${base_url}${result.poster_path}`: `${placeholder}`
              } />
            </div>
            <div className='card__block-inner'>
              <div>
                <h3 className='card__title'>{result.title}</h3>	
                <p className='card__description'><span>{result.overview}</span></p>									
              </div>
              <div className="card__extra-info">
                <span className='card__rating'>Rating: {isNaN(result.vote_average) ? 0 : Math.ceil(( result.vote_average * 10 ))}%</span> 
        ...
        </ul>: <p className='no-results'>Sorry not results found..</p>;
  }
